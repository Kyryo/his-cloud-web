...
```mermaid
flowchart TD
    A[Prospect] --> B[Research]
    B --> C[Cold Call]
    C --> D{Qualified?}
    D -- No --> D1[Not a Fit\nRecord in CRM]
    D -- Yes --> E[Clinic Operations Review\nDiscovery + Tailored Demo]
    E --> F{Interest Confirmed?}
    F -- No --> F1[Follow-up Scheduled\nNurture]
    F -- Yes --> G[Pilot Discussion]
    G --> H[Letter of Intent Signed]
    H --> I[Pilot]
    I --> J[Pilot Review]
    J --> K{Success Criteria Met?}
    K -- Yes --> L[Commercial Proposal]
    K -- No --> K1[Address Gaps\nExtend or Close Out]
    L --> M[Contract]
    M --> N[Implementation]
```
...
```mermaid
flowchart TD
    S([Call Connected]) --> I[Introduction:\nask for two minutes]
    I --> T{Good time?}
    T -- No --> R[Ask for a better time] --> O1[Outcome:\nFollow-up Scheduled]
    T -- Yes --> C[Mention common\noperational challenges]
    C --> F{Sounds familiar?}
    F -- Yes --> DQ[Targeted discovery\nquestions]
    DQ --> TR[Invite them to a\nClinic Operations Review]
    TR --> O2[Outcome:\nReview Scheduled]
    F -- No --> CS[Ask about current\nsystem]
    CS --> IM{Anything they'd\nimprove?}
    IM -- Yes --> TR
    IM -- No --> O3[Outcome:\nNot a Fit]
```
...
```mermaid
flowchart LR
    P[Problem] --> D[Discussion] --> RF[Relevant Feature] --> BO[Business Outcome]
```
...
```mermaid
flowchart TD
    A[Letter of Intent Signed] --> B[Pilot Kickoff]
    B --> C[Weekly Check-ins\nwith Users]
    C --> D[Weekly Pilot Report]
    D --> C
    D --> E[Pilot Review]
    E --> F{Success Criteria Met?}
    F -- Yes --> G[Commercial Proposal]
    F -- No --> H[Address Gaps\nExtend or Close Out]
```

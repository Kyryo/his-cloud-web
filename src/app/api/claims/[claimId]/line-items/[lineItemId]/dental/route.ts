...
export async function PUT(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const claim = await hmisApiRequest<ClaimDetail>(
      CLAIMS_API_PATHS.lineItemDental(claimId, lineItemId),
      {
        method: "PUT",
    ...
    if (error instanceof HmisApiError) {
      console.error("[claims/line-items/dental] HMIS API error", {
        claimId,
        lineItemId,
        status: error.status,
      ...
      console.error("[claims/line-items/dental] Unexpected error", {
        claimId,
        lineItemId,
        error,
      });

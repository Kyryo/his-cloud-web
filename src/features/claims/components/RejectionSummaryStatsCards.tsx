...
type RejectionSummaryStatsCardsProps = {
  stats: RemittanceRejectionSummaryStats | null;
  isLoading?: boolean;
};

const EMPTY_BUCKET: RemittanceBatchRowStatsBucket = { count: 0, total: "0" };

export function RejectionSummaryStatsCards({
  stats,
  isLoading = false,
}: RejectionSummaryStatsCardsProps) {
  const buckets = stats ?? {
    claimed: EMPTY_BUCKET,
    pay_to_provider: EMPTY_BUCKET,
    matched: EMPTY_BUCKET,
    with_reason: EMPTY_BUCKET,
  };

  return (
    <StatsCard1Grid data-testid="rejection-summary-stats">
      <StatsCard1
      ...
      />
      <StatsCard1
        title="With reason code"
        icon="transfer"
        tone="rose"
        isLoading={isLoading}
        value={
          <ListPageCountAmountValue
            count={buckets.with_reason.count}
            total={buckets.with_reason.total}
          />
        }
      />
    </StatsCard1Grid>

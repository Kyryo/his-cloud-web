...
function ClientTabSkeleton() {
  return (
    <div className="space-y-6" data-testid="sales-order-client-skeleton">
...
export function SalesOrderDetailClientTab({
  ...
  return (
    <DetailClientProfile
      ...
      data-testid="sales-order-client-tab"
      outstanding={
        billing
      ...
      paid={
        billing
          ? formatSalesOrderAmount(billing.totals.total_paid, currency)
          : undefined
      }
      ...
      coverage={coverage.map((record) => ({
        ...
        onThisDocument: isOrderCoverage(record, order) ? "On this order" : null,
      }))}
    />
  );
}
